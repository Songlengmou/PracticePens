...
public class SplineCurveStrategy {
    ...
    public void drawPen(DrawingStrokes drawingStrokes) {
        ...
        if (drawingStrokes.isUp) {
            ...
            if (drawSteps > 100) {
                curveIndex = 40;
            } else if (drawSteps > 80) {
                curveIndex = 35;
            } else if (drawSteps > 70) {
                curveIndex = 30;
            } else if (drawSteps > 60) {
                curveIndex = 25;
            } else if (drawSteps > 50) {
                curveIndex = 20;
            } else if (drawSteps > 40) {
                curveIndex = 15;
            } else if (drawSteps > 30) {
                curveIndex = 13;
            } else if (drawSteps > 20) {
                curveIndex = 9;
            } else if (drawSteps > 10) {
                curveIndex = 7;
            } else if (drawSteps >= 4) {
                curveIndex = 3;
            } else {
...
//        if(curveIndex==0) curveIndex=1;
        for (int i = 0, num = 1; i < drawSteps; i += curveIndex, num++) {
            ...
            if (numX > 0 && numY > 0) {
                currentState = DrawingStrokes.X_ADD_Y_ADD;
            }
            if (numX > 0 && numY < 0) {
                currentState = DrawingStrokes.X_ADD_Y_DEC;
            }
            if (numX > 0 && numY == 0) {
                currentState = DrawingStrokes.X_ADD_Y_SAM;
            }
            if (numX < 0 && numY > 0) {
                currentState = DrawingStrokes.X_DEC_Y_ADD;
            }
            if (numX < 0 && numY < 0) {
                currentState = DrawingStrokes.X_DEC_Y_DEC;
            }
            if (numX < 0 && numY == 0) {
                currentState = DrawingStrokes.X_DEC_Y_SAM;
            }
            if (numX == 0 && numY > 0) {
                currentState = DrawingStrokes.X_SAM_Y_ADD;
            }
            if (numX == 0 && numY < 0) {
                currentState = DrawingStrokes.X_SAM_Y_DEC;
            }
            if (numX == 0 && numY == 0) {
                currentState = DrawingStrokes.X_SAM_Y_SAM;
            }
...
//                    mPath.lineTo(myPointA.x, myPointA.y);
                    ...
                    if (isAC) {
                        ...
                        mPath.quadTo(centerBD.x, centerBD.y, myPointA.x, myPointA.y);
                    }
                    mPath.lineTo(myPointC.x, myPointC.y);
                    mPath.lineTo(myPointD.x, myPointD.y);
